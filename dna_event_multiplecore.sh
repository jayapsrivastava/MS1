...
set -x
	...
	numberOfCores=12
        ...
        do
			...
			cp $rc slave.$machine
			coreCount=0	
			UDPPortNo=`expr $portNo + 100`
			while [ $coreCount -le $numberOfCores ]; 
			do
                        	srun -p $partition --nodelist $machine --exclusive ./slave.$machine slave $ipAdd $UDPPortNo +RTS -l-au & 
	...
	while [ $coreCount -le $numberOfCores ]; 
	do
       		srun -p $partition --nodelist $getMasterIP --exclusive ./master.$getMasterIP master $ipAdd $UDPPortNo +RTS -l-au 
		UDPPortNo=`expr $UDPPortNo + 1`
                coreCount=`expr $coreCount + 1`
	done
