...
set -x
	...
	numberOfCores=12
        ...
        do
			...
			do
                        	srun -p $partition --nodelist $machine -N1 -n1 --exclusive ./slave.$machine slave $ipAdd $UDPPortNo +RTS -l-au & 
	...
	while [ $coreCount -le $numberOfCores ]; 
	do
       		srun -p $partition --nodelist $getMasterIP -N1 -n1 --exclusive ./master.$getMasterIP master $ipAdd $UDPPortNo +RTS -l-au 
		UDPPortNo=`expr $UDPPortNo + 1`
                coreCount=`expr $coreCount + 1`
	done
