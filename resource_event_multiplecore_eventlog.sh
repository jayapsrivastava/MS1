...
$PWD/dna_event_multiplecore.sh check $rc $JOBID $slurmJobNodeList 0
if [ $? -eq 0 ]; then
...
$PWD/dna_event_multiplecore.sh generatingCADFile $rc $JOBID $slurmJobNodeList $SLURM_NTASKS_PER_NODE
if [ $? -eq 0 ]; then
...
$PWD/dna_event_multiplecore.sh md5sum $rc 0 0 0
if [ $? -eq 0 ]; then
...
$PWD/dna_event_multiplecore.sh startingCHprocess $rc $JOBID machine.file.$JOBID $SLURM_NTASKS_PER_NODE
if [ $? -eq 0 ]; then
...
$PWD/dna_event_multiplecore.sh collectLogs 0 $JOBID $slurmJobNodeList $SLURM_NTASKS_PER_NODE
if [ $? -eq 0 ]; then
