...
$PWD/dna.sh check $rc $JOBID $slurmJobNodeList
if [ $? -eq 0 ]; then
...
$PWD/dna.sh generatingCADFile $rc $JOBID $slurmJobNodeList 
if [ $? -eq 0 ]; then
...
$PWD/dna.sh startingCHprocess $rc $JOBID machine.file.$JOBID 
if [ $? -eq 0 ]; then
...
$PWD/dna.sh collectLogs 0 $JOBID $slurmJobNodeList
if [ $? -eq 0 ]; then
