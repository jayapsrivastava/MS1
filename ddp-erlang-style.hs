...
spawnCompute (file, chOffset, chSize, itemCount, collectorPID, opert) = do
	...
	case opert of
		"crash" -> do	
			sayDebug $ printf "Compute [%s] asking master[%s] for crash" (show computePID) (show masterPID)
			send masterPID (WhomToCrash computePID)
			(WhomToCrash masterPID) <- expect
			sayDebug $ printf "Specified Compute [%s] crashed already!!" (show computePID)
		"norm" -> do
			sayDebug $ printf "Normal Computation %s" (show computePID)
	...
	case opert of
		"crash" -> do
			forM_ computePids $ \computePid -> do
				...
				then do 
					(WhomToCrash computeToCrash) <- expect
					send computeToCrash (CrashMe masterPID)
					sayDebug $ printf "Compute [%s] has to crash!!"  (show computeToCrash)
				else do (WhomToCrash computePid) <- expect
					send computePid (WhomToCrash masterPID)
					sayDebug $ printf "Compute [%s] continuing the computation!!"  (show computePid)
		"norm" -> do
			sayDebug $ printf "Continue with computation!!"
