...
binPath=$HOME/jaya/git_erlang/RC/MS1/ddp-erlang-style
...
function generatingCADFile {
	...
	for machine in `cat $slurmJobNodeList`
		...
		coreCount=1	
		while [ $coreCount -le $ntaskspernode ]; 
		do
	...
	totalmachineCount=`expr $machineCount \* $ntaskspernode`
...
function startingCHprocess {
	...
	noOfComputeP=$(($noOfComputeP * $ntaskspernode))
	itemCount=`expr $noOfComputeP \* 15000000`
	itemCounti=`expr $ntaskspernode \* 150`
    ...
    chunkSize=0
        ...
        do
			...
			do
				#cp slave.$machine slave.$machine.$eventLog
				cp $rc slave.$machine.$eventLog
				echo "THE VAL = $line \n"
                ...
                srun -p $partition --nodelist $machine -N1 -n1 --exclusive ./slave.$machine.$eventLog slave --cad $PWD/CAD.$JOBID.file --ip $ipAdd --port $portNo +RTS -l-au &
				eventLog=`expr $eventLog + 1`
...
#	cp $rc master.$getMasterIP	
	...
	while read -u 3 line
        do
		#cp master.$getMasterIP master.$getMasterIP.$eventLog
		cp $rc master.$getMasterIP.$eventLog
		portNo=`echo $line | cut -f2 -d":"`
		srun -p $partition --nodelist $getMasterIP -N1 -n1 --exclusive ./create-floats /ramdisks/file.$getMasterIP $itemCount $noOfComputeP 0
        	echo -e "\n============\nStarting Master====<$ipAdd>=====<$portNo>=====\n============\n"
       		srun -p $partition --nodelist $getMasterIP -N1 -n1 --exclusive ./master.$getMasterIP.$eventLog master --cad $PWD/CAD.$JOBID.file --ip $ipAdd --port $portNo --filename $HOME/input_data.txt +RTS -l-au 
		eventLog=`expr $eventLog + 1`
	done 3< $PWD/temp_CAD.txt
	rm -rf $PWD/temp_CAD.txt
...
function collectLogs {
	...
	do
		...
		if [ $flag -eq 0 ]
                then
			mkdir -p $PWD/Logs_$JOBID/Master-$machine-eventLogPerThread
			while [ $eventLog -le $ntaskspernode ]; 
			do
				ghc-events show $PWD/master.$machine.$eventLog.eventlog > $PWD/Logs_$JOBID/master.$machine.$eventLog.eventlog.txt
				mv $PWD/Logs_$JOBID/master.$machine.$eventLog.eventlog.txt $PWD/Logs_$JOBID/Master-$machine-eventLogPerThread/.
				mv $PWD/master.$machine.$eventLog.eventlog $PWD/Logs_$JOBID/Master-$machine-eventLogPerThread/.
				mv $PWD/master.$machine.$eventLog $PWD/Logs_$JOBID/Master-$machine-eventLogPerThread/.
				eventLog=`expr $eventLog + 1`
			done
			#rm $PWD/master.$machine 
			flag=1
		else
			mkdir -p $PWD/Logs_$JOBID/Slave-$machine-eventLogPerThread
			while [ $eventLog -le $ntaskspernode ]; 
			do
				ghc-events show $PWD/slave.$machine.$eventLog.eventlog > $PWD/Logs_$JOBID/slave.$machine.$eventLog.eventlog.txt
				mv $PWD/Logs_$JOBID/slave.$machine.$eventLog.eventlog.txt $PWD/Logs_$JOBID/Slave-$machine-eventLogPerThread/.
				mv $PWD/slave.$machine.$eventLog.eventlog $PWD/Logs_$JOBID/Slave-$machine-eventLogPerThread/.
				mv $PWD/slave.$machine.$eventLog $PWD/Logs_$JOBID/Slave-$machine-eventLogPerThread/.
				eventLog=`expr $eventLog + 1`
			done
